import java.util.*;
public class transformBackFromLeftCloned {
    static class Node{
        int data;
         Node left = null;
         Node right = null;
    ...
    public static Node reverseleftCloned(Node root){
        if(root == null){
            return root;
        }

        Node lc = reverseleftCloned(root.left.left);   //left cloned
        Node rc = reverseleftCloned(root.right);  //right cloned
        root.right = rc; 
        root.left = lc;
